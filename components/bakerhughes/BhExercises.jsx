          ...
          return (
            <button key={i} onClick={() => toggle(i)} disabled={checked}
              style={{ display: 'flex', alignItems: 'flex-start', gap: 10, textAlign: 'left', padding: '12px 14px', borderRadius: 10, cursor: checked ? 'default' : 'pointer', fontSize: 14.5, lineHeight: 1.5, fontFamily: 'inherit',
                background: state === 'ok' ? '#F0FFF4' : state === 'bad' ? '#FFF5F5' : on ? c.accentLight || '#E4F7EC' : '#fff',
                border: `2px solid ${state === 'ok' ? '#9AE6B4' : state === 'bad' ? '#FEB2B2' : on ? accent : c.grayLight || '#E2E9E7'}`,
                color: c.text || '#20302D' }}>
              ...
              <span style={{ flex: 1 }}>{o.text}{checked && o.correct && <strong style={{ color: '#2F855A' }}> · correta</strong>}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'True or false'}>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {items.map((it, i) => (
          <div key={i} style={{ padding: '12px 14px', borderRadius: 10, background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : c.offWhite || '#F5F8F7', border: `1px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}` }}>
            ...
            {checked && !isRight(i) && it.why && <div style={{ marginTop: 9, fontSize: 13.5, color: '#742A2A' }}>{it.why}</div>}
          </div>
        ))}
      </div>
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Classifique'}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
        {cats.map((cat) => (
          <span key={cat.id} style={{ padding: '5px 12px', borderRadius: 999, background: c.accentLight || '#E4F7EC', color: c.navy || '#062E2B', fontSize: 13, fontWeight: 700 }}>{cat.name}</span>
        ))}
      </div>
      <div style={{ display: 'grid', gap: 9 }}>
        {order.map((i) => {
          const it = items[i];
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', padding: '11px 14px', borderRadius: 10, background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : '#fff', border: `1px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}` }}>
      ...
      <div style={{ display: 'grid', gap: 7, marginBottom: 12 }}>
        {seq.map((idx, pos) => (
          <div key={pos} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', borderRadius: 10,
            background: checked ? (isRight(pos) ? '#F0FFF4' : '#FFF5F5') : c.accentLight || '#E4F7EC',
            border: `1px solid ${checked ? (isRight(pos) ? '#9AE6B4' : '#FEB2B2') : accent}` }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {pool.filter((i) => !seq.includes(i)).map((i) => (
          <button key={i} onClick={() => place(i)} disabled={checked}
            style={{ padding: '9px 14px', borderRadius: 10, border: `1px solid ${c.grayLight || '#E2E9E7'}`, background: '#fff', color: c.text || '#20302D', fontSize: 14, fontFamily: 'inherit', textAlign: 'left', cursor: checked ? 'default' : 'pointer', maxWidth: '100%', lineHeight: 1.45 }}>
            {label(correct[i])}
          </button>
        ))}
      </div>
        ...
        {!checked ? (
          <>
            <button onClick={() => { setChecked(true); if (onChecked) onChecked(acc); }} disabled={seq.length !== correct.length}
              style={{ padding: '10px 18px', borderRadius: 8, border: 'none', background: seq.length === correct.length ? accent : '#CBD5D2', color: '#fff', fontWeight: 700, fontSize: 14, cursor: seq.length === correct.length ? 'pointer' : 'not-allowed' }}>Corrigir</button>
            {seq.length > 0 && <button onClick={undo} style={{ padding: '10px 18px', borderRadius: 8, border: `1px solid ${c.grayLight || '#E2E9E7'}`, background: '#fff', fontWeight: 700, fontSize: 14, fontFamily: 'inherit', cursor: 'pointer' }}>Voltar uma</button>}
          </>
        ) : (
          <button onClick={() => { setChecked(false); setSeq([]); }} style={{ padding: '10px 18px', borderRadius: 8, border: `1px solid ${c.grayLight || '#E2E9E7'}`, background: '#fff', fontWeight: 700, fontSize: 14, fontFamily: 'inherit', cursor: 'pointer' }}>Tentar de novo</button>
        )}
      </div>
                  ...
                  return (
                    <button key={ti} onClick={() => !checked && setPick((p) => ({ ...p, [i]: clean }))} disabled={checked}
                      style={{ padding: '2px 6px', borderRadius: 6, fontSize: 15, fontFamily: 'inherit', cursor: checked ? 'default' : 'pointer',
                        background: state === 'ok' ? '#C6F6D5' : state === 'bad' ? '#FED7D7' : on ? c.accentLight || '#E4F7EC' : 'transparent',
                        border: `1px solid ${on || state ? (state === 'bad' ? '#FEB2B2' : accent) : 'transparent'}`, color: c.text || '#20302D' }}>{t}</button>
                  );
                })}
              </div>
              {checked && <div style={{ marginTop: 8, fontSize: 13.5, color: isRight(i) ? '#22543D' : '#742A2A' }}><strong>{it.wrong} → {it.fix}</strong>{it.why ? ` · ${it.why}` : ''}</div>}
            </div>
          );
        })}
      </div>
          ...
          return (
            <button key={i} onClick={() => !checked && setOn((o) => ({ ...o, [i]: !o[i] }))} disabled={checked}
              style={{ padding: '2px 6px', borderRadius: 6, fontSize: 15.5, fontFamily: 'inherit', cursor: checked ? 'default' : 'pointer',
                background: state === 'ok' ? '#C6F6D5' : state === 'bad' ? '#FED7D7' : on[i] ? c.accentLight || '#E4F7EC' : 'transparent',
                border: `1px solid ${on[i] || state ? (state === 'bad' ? '#FEB2B2' : accent) : 'transparent'}`, color: c.text || '#20302D' }}>{t}</button>
          );
        })}
      </div>
            ...
            {i < gaps.length && (
              <select value={ans[i] || ''} onChange={(e) => setAns((a) => ({ ...a, [i]: e.target.value }))} disabled={checked}
                style={{ margin: '0 4px', padding: '4px 8px', borderRadius: 6, fontSize: 14.5, fontFamily: 'inherit',
                  border: `2px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}`,
                  background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : '#fff' }}>
                <option value="">…</option>
...
function SerialChoice({ ex, c, onChecked }) {
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Escolha certa'}>
      ...
      <div style={{ display: 'grid', gap: 14 }}>
        {items.map((it, i) => (
          <div key={i} style={{ padding: '12px 14px', borderRadius: 10, background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : '#fff', border: `1px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}` }}>
            <div style={{ fontSize: 14.5, lineHeight: 1.6, marginBottom: 9, color: c.navy || '#062E2B', fontWeight: 600 }}>{it.prompt}</div>
                ...
                return (
                  <button key={oi} onClick={() => !checked && setPick((p) => ({ ...p, [i]: oi }))} disabled={checked}
                    style={{ padding: '8px 14px', borderRadius: 999, fontSize: 14, fontWeight: 600, fontFamily: 'inherit', cursor: checked ? 'default' : 'pointer', textAlign: 'left',
                      ...
                      border: `1.5px solid ${state === 'ok' ? '#9AE6B4' : state === 'bad' ? '#FEB2B2' : on ? accent : c.grayLight || '#E2E9E7'}`,
...
function FlowChoice({ ex, c, onChecked, voiceType }) {
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Conversa'}>
                ...
                return (
                  <button key={oi} onClick={() => choose(i, oi)} disabled={answered}
                    style={{ textAlign: 'left', padding: '10px 14px', borderRadius: 12, fontSize: 14.5, lineHeight: 1.5, fontFamily: 'inherit', cursor: answered ? 'default' : 'pointer',
                      background: state === 'ok' ? '#F0FFF4' : state === 'bad' ? '#FFF5F5' : '#fff',
                      border: `1.5px solid ${state === 'ok' ? '#9AE6B4' : state === 'bad' ? '#FEB2B2' : c.grayLight || '#E2E9E7'}`,
                      color: c.text || '#20302D' }}>
                    {o.text}
                    {answered && on && o.why && <div style={{ marginTop: 6, fontSize: 13, color: o.correct ? '#22543D' : '#742A2A' }}>{o.why}</div>}
                  </button>
                );
              })}
            </div>
      ...
      {done && (
        <div style={{ marginTop: 16, padding: '12px 15px', borderRadius: 10, background: acc === 1 ? '#F0FFF4' : c.accentLight || '#E4F7EC', border: `1px solid ${acc === 1 ? '#9AE6B4' : accent}`, fontSize: 14.5 }}>
          <strong>{score}/{turns.length}</strong> — {ex.explanation || 'Conversa concluída.'}
          <button onClick={() => { setPicks({}); setStep(0); }} style={{ marginLeft: 12, padding: '6px 14px', borderRadius: 8, border: `1px solid ${c.grayLight || '#E2E9E7'}`, background: '#fff', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>Refazer</button>
        </div>
      )}
    </ExShell>
  );
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Ouça e complete'}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 14 }}>
        {(ex.bank || []).map((w, i) => (
          <span key={i} style={{ padding: '5px 12px', background: '#fff', border: `1px solid ${c.accent || '#00B04F'}`, borderRadius: 999, fontSize: 13.5, fontWeight: 600, color: c.navy || '#062E2B' }}>{w}</span>
        ))}
      </div>
      <div style={{ display: 'grid', gap: 12 }}>
        {items.map((it, i) => {
    ...
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Monte a frase'}>
      ...
      <div style={{ display: 'grid', gap: 18 }}>
        {items.map((it, i) => {
          ...
          return (
            <div key={i} style={{ padding: '13px 14px', borderRadius: 10, background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : c.offWhite || '#F5F8F7', border: `1px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}` }}>
              {it.hint && <div style={{ fontSize: 13.5, color: c.gray || '#5F7570', marginBottom: 9, fontStyle: 'italic' }}>{it.hint}</div>}
              <div style={{ minHeight: 40, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', padding: '8px 10px', background: '#fff', border: `1px dashed ${c.grayLight || '#E2E9E7'}`, borderRadius: 8, marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {remaining.map((w, k) => (
                  <button key={k} onClick={() => add(i, w, k)} disabled={checked}
                    style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${c.grayLight || '#E2E9E7'}`, background: '#fff', fontSize: 14.5, fontFamily: 'inherit', cursor: checked ? 'default' : 'pointer', color: c.text || '#20302D' }}>{w}</button>
                ))}
                {used.length > 0 && !checked && (
                  <button onClick={() => back(i)} style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${c.grayLight || '#E2E9E7'}`, background: '#fff', fontSize: 13, fontFamily: 'inherit', cursor: 'pointer', color: c.gray || '#5F7570' }}>← apagar</button>
                )}
              </div>
              {checked && (
                <div style={{ marginTop: 10, fontSize: 14, color: isRight(i) ? '#22543D' : '#742A2A', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Leia e responda'}>
      ...
      <div style={{ display: 'grid', gap: 14 }}>
        {qs.map((q, i) => (
          <div key={i} style={{ padding: '12px 14px', borderRadius: 10, background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : '#fff', border: `1px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}` }}>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: c.navy || '#062E2B', marginBottom: 9, lineHeight: 1.55 }}>{i + 1}. {q.prompt}</div>
            <div style={{ display: 'grid', gap: 7 }}>
              {(q.options || []).map((o, oi) => {
                const on = pick[i] === oi;
                const state = checked ? (o.correct ? 'ok' : on ? 'bad' : null) : null;
  ...
  return (
    <ExShell image={ex.image} imageCaption={ex.imageCaption} title={ex.title} c={c} badge={ex.badge || 'Caixa de entrada'}>
      ...
      <div style={{ border: `1px solid ${c.grayLight || '#E2E9E7'}`, borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
        ...
        <div style={{ padding: '15px 17px', background: '#fff', fontSize: 14.8, lineHeight: 1.75, whiteSpace: 'pre-line', color: c.text || '#20302D' }}>{m.body}</div>
      </div>
      <div style={{ display: 'grid', gap: 14 }}>
        {qs.map((q, i) => (
          <div key={i} style={{ padding: '12px 14px', borderRadius: 10, background: checked ? (isRight(i) ? '#F0FFF4' : '#FFF5F5') : c.offWhite || '#F5F8F7', border: `1px solid ${checked ? (isRight(i) ? '#9AE6B4' : '#FEB2B2') : c.grayLight || '#E2E9E7'}` }}>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: c.navy || '#062E2B', marginBottom: 9, lineHeight: 1.55 }}>{q.prompt}</div>
            <div style={{ display: 'grid', gap: 7 }}>
              {(q.options || []).map((o, oi) => {
                const on = pick[i] === oi;
                const state = checked ? (o.correct ? 'ok' : on ? 'bad' : null) : null;
                return (
                  <button key={oi} onClick={() => !checked && setPick((p) => ({ ...p, [i]: oi }))} disabled={checked}
                    style={{ textAlign: 'left', padding: '9px 13px', borderRadius: 10, fontSize: 14.5, lineHeight: 1.5, fontFamily: 'inherit', cursor: checked ? 'default' : 'pointer',
                      background: state === 'ok' ? '#C6F6D5' : state === 'bad' ? '#FED7D7' : on ? '#fff' : '#fff',
                      border: `1.5px solid ${state === 'ok' ? '#9AE6B4' : state === 'bad' ? '#FEB2B2' : on ? accent : c.grayLight || '#E2E9E7'}`,
                      color: c.text || '#20302D' }}>{o.text}</button>
                );
              })}
                ...
                return (
                  <button key={k} onClick={() => !checked && setPick((p) => ({ ...p, [i]: k }))} disabled={checked}
                    style={{ textAlign: 'left', padding: '12px 14px', borderRadius: 12, fontSize: 14.5, lineHeight: 1.55, fontFamily: 'inherit', cursor: checked ? 'default' : 'pointer',
                      background: state === 'ok' ? '#C6F6D5' : state === 'bad' ? '#FED7D7' : on ? c.accentLight || '#E4F7EC' : c.offWhite || '#F5F8F7',
                      border: `1.5px solid ${state === 'ok' ? '#9AE6B4' : state === 'bad' ? '#FEB2B2' : on ? accent : c.grayLight || '#E2E9E7'}`,
                      color: c.text || '#20302D' }}>
...
function CheckOff({ ex, c, voiceType, onChecked }) {
  ...
  return (
    <div style={{ background: '#fff', border: `2px solid ${all ? accent : c.grayLight || '#E2E9E7'}`, borderRadius: 14, padding: '22px 24px', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
        ...
        <h4 style={{ margin: 0, fontSize: 16.5, color: navy }}>{ex.title || 'Antes de fechar a lição'}</h4>
      </div>
        ...
        {items.map((it, i) => {
          ...
          return (
            <button key={i} onClick={() => setOn((o) => ({ ...o, [i]: !o[i] }))}
              ...
              <span style={{ flex: 1 }}>
                <span style={{ display: 'block', fontSize: 14.8, color: navy, fontWeight: 600, lineHeight: 1.45 }}>{text}</span>

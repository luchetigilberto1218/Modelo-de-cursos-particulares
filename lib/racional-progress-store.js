...
// Quando cada aluno sincronizou pela última vez, para a leitura de turma da
// coordenação. O doc da Racional guarda só o mapa de lições — sem carimbo de
// data lá dentro —, então a data que existe é a do próprio arquivo no Blob.
// ADITIVO: exportação nova, nada do que já existia muda.
export async function listUpdatedAt() {
  const out = new Map();
  let cursor;
  do {
    const res = await list({ prefix: 'progress/', cursor, limit: 1000 });
    for (const b of res.blobs) {
      const id = b.pathname.replace(/^progress\//, '').replace(/\.json$/, '');
      out.set(id, b.uploadedAt instanceof Date ? b.uploadedAt.toISOString() : b.uploadedAt || null);
    }
    cursor = res.cursor;
  } while (cursor);
  return out;

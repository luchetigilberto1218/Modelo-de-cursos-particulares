...
import { getSession, canAccessClient, isDisabledSession } from './auth';
// Only these client routes require login. Everything else stays open,
// exactly like before. A lista mora em `primary-client` (módulo puro) para que
// a tela de login possa consultar a MESMA lista ao validar o `?next=`.
import { PROTECTED_CLIENTS } from './primary-client';
...
export async function guardClient(client) {
  ...
  // Conta desativada depois que o cookie foi emitido: corta o acesso na hora.
  if (isDisabledSession(session)) {

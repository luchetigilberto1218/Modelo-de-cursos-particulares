...
async function bloco(id, nome, href, painel, carregar, aviso = null) {
  try {
    return { id, nome, href, painel, aviso, erro: null, alunos: await carregar() };
  } catch (e) {
    return { id, nome, href, painel, aviso, erro: e?.message || 'falha ao ler', alunos: [] };
  }
}
  ...
  return getUsers()
    ...
    .map((u) => {
      ...
      return {
        ...
        detalhe: u.disabled ? 'acesso desativado' : 'trilha pessoal',
        inativo: !!u.disabled,
      };
    });
}
...
export async function getPainelCoordenacao() {
  return Promise.all([
    bloco('czarnikow', 'Czarnikow', '/czarnikow', '/czarnikow/professor', alunosCzarnikow),
    ...
    bloco('bakerhughes', 'Baker Hughes', '/bakerhughes', null, alunosBakerHughes),
  ]);
}

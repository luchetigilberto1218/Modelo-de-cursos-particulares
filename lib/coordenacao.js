...
import { listAll as listAsta } from './asta-progress-store';
import { alunosEurochem, alunosSucafina } from './externos';
  ...
  return getUsers()
    ...
    .map((u) => {
      ...
      return {
        ...
        detalhe: u.disabled ? 'saiu do programa'
          : [u.stage || u.level, trilhas.map((t) => nomeTrilha.get(t) || t).join(', ')]
            .filter(Boolean).join(' · '),
        inativo: !!u.disabled,
      };
    });
...
async function alunosRacional() {
  ...
  // Um aluno por vez somava as esperas; em paralelo, o bloco sai no tempo do mais lento.
  const docs = await Promise.all((idx.students || []).map((s) => readRacional(s.id)));
  const alunos = [];
  for (const [i, s] of (idx.students || []).entries()) {
    const p = docs[i];
  ...
  return getUsers()
    ...
    .map((u) => {
      ...
      return {
        ...
        meta: total,
        ultimaAt: ultimaAtividade(doc),
        // Quem saiu do grupo de estudo continua no painel, marcado — o
        // histórico não some.
        detalhe: u.disabled ? 'saiu do grupo de estudo' : (u.level || ''),
        inativo: !!u.disabled,
      };
    });
}
...
// Mesmo formato do Baker Hughes: trilha pessoal (`owner`) é a meta de quem a
// tem. Quem ainda não mandou os temas estuda nas trilhas compartilhadas e fica
// sem meta até a trilha dele entrar. RH e apresentador não são alunos.
async function alunosAsta() {
  const docs = await listAsta();
  const porAluno = new Map(docs.map((d) => [d.student, d]));
  const course = getCourse(ASTA_CLIENT);
  const lessons = course?.lessons || [];
  const nomeTrilha = new Map((course?.tracks || []).map((t) => [t.id, t.name]));
  const daTrilha = (owner) => lessons.filter((l) => l.track === owner).length || null;

  return getUsers()
    .filter((u) => u.role === 'student' && (u.clients || []).includes(ASTA_CLIENT))
    .map((u) => {
      const doc = porAluno.get(u.id) || null;
      const owner = (u.tracks || [])[0] || null;
      return {
        ...
        meta: owner ? daTrilha(owner) : null,
        ultimaAt: ultimaAtividade(doc),
        detalhe: u.disabled ? 'acesso desativado'
          : owner ? (nomeTrilha.get(owner) || 'trilha pessoal')
          : 'no aguardo dos temas · trilhas compartilhadas',
        inativo: !!u.disabled,
      };
    });
}
...
export async function getPainelCoordenacao() {
  return Promise.all([
    ...
    bloco('asta', 'ASTA', '/asta', null, alunosAsta,
      'Até 22/09/2026 a conclusão das lições da ASTA não era gravada no servidor. O que cada pessoa fez antes disso não aparece aqui.',
      SO_ALUNO),
    // Os dois abaixo moram na Vercel da Helen: leitura só, ver lib/externos.js.
    bloco('eurochem', 'EuroChem', 'https://eurochem-material.vercel.app', null, alunosEurochem,
      'Material fora deste projeto (Vercel da Helen), lido sem gravar nada lá. Conta as preparações de aula enviadas; o professor marca a aula dada no próprio material.',
      'o material tem contador próprio — não entra aqui'),
    bloco('sucafina', 'Sucafina · Daniela', 'https://sucafina-portugues.vercel.app', null, alunosSucafina,
      'Material fora deste projeto (Vercel da Helen), lido sem gravar nada lá. Conta as preparações de aula enviadas.',
      'o material tem contador próprio — não entra aqui'),
  ]);
}
